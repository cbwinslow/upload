...
def fetch_extensions(config: InstallerConfig) -> List[Extension]:
    ...
    This format is intentionally generic so you can adapt it to the
    actual Gemini marketplace API by tweaking this function only.
    """
...
def filter_extensions(
    ...
    - tag: keep only extensions whose tags contain the given tag.
    - category: keep only extensions whose category matches (case-insensitive).
    - name_pattern: regex pattern matched against the name or id.
    """
...
def extension_needs_install(ext: Extension, state: Dict[str, Any]) -> bool:
    ...
    Returns True if the extension is not present in the state or if the
    version differs. This intentionally does not attempt semantic
    versioning comparisons; it simply compares strings for simplicity.
    """
...
def install_extension(ext: Extension, config: InstallerConfig) -> bool:
    ...
    try:
        # Use shell=True to allow complex command templates, but this
        # should be used carefully. The template should not include
        # untrusted user input besides the extension id.
...
def sync_extensions(
    ...
    if failed_count > 0:
        # Non-zero exit code to signal partial failure to calling tools.
        raise SystemExit(1)
